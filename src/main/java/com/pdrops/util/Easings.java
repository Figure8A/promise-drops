...
public class Easings {
    ...
    private static final float c4 = (float) ((2 * MathHelper.PI) / 3);
    private static final float c5 = (float) ((2 * MathHelper.PI) / 4.5);
    private static final float n1 = 7.5625f;
    private static final float d1 = 2.75f;


    public static float inSine(float f){
        return (float) (1 - MathHelper.cos((f * MathHelper.PI) / 2));
    }

    public static float outSine(float f){
        return MathHelper.sin((f * MathHelper.PI) / 2);
    }

    public static float inOutSine(float f){
        return (float) (- (MathHelper.cos(MathHelper.PI * f) - 1) / 2);
    }
    ...
    public static float inCirc(float x){
        return (float) (1 - MathHelper.sqrt((float) (1 - Math.pow(x, 2))));
    }

    public static float outCirc(float x){
        return (float) (MathHelper.sqrt((float) (1 - Math.pow(x - 1, 2))));
    }

    public static float inOutCirc(float x){
        return (float) (x < 0.5 ? (1 - MathHelper.sqrt((float) (1 - Math.pow(2 * x, 2)))) / 2 : (MathHelper.sqrt((float) (1 - Math.pow(- 2 * x + 2, 2))) + 1) / 2);
    }
    ...
    public static float inElastic(float x){
        return (float) (x == 0 ? 0 : x == 1 ? 1 : - Math.pow(2, 10 * x - 10) * MathHelper.sin((float) ((x * 10 - 10.75) * c4)));
    }

    public static float outElastic(float x){
        return (float) (x == 0 ? 0 : x == 1 ? 1 : Math.pow(2, - 10 * x) * MathHelper.sin((float) ((x * 10 - 0.75) * c4)) + 1);
    }

    public static float inOutElastic(float x){
        return (float) (x == 0 ? 0 : x == 1 ? 1 : x < 0.5 ? - (Math.pow(2, 20 * x - 10) * MathHelper.sin((float) ((20 * x - 11.125) * c5))) / 2 : (Math.pow(2, - 20 * x + 10) * MathHelper.sin((float) ((20 * x - 11.125) * c5))) / 2 + 1);
    }
